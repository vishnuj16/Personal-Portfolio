...
const EDU_FIELDS = [
  ...
  { key: 'ended_at', label: 'ended_at', placeholder: 'YYYY-MM or blank if ongoing' },
]

function EduCard({ edu, isEditMode, onEdit }) {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        width: hovered ? 180 : 80, height: hovered ? 180 : 80,
        background: 'radial-gradient(circle at top right, rgba(0,229,255,0.08) 0%, transparent 70%)',
        transition: 'all 0.4s ease',
        pointerEvents: 'none',
      }} />

      {/* Bottom-left cyan line accent */}
      <div style={{
        position: 'absolute', bottom: 0, left: 0, right: 0, height: 2,
        background: hovered
          ? 'linear-gradient(to right, var(--cyan), transparent)'
          : 'linear-gradient(to right, rgba(0,229,255,0.2), transparent)',
        transition: 'all 0.3s',
      }} />
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, position: 'relative' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {(edu.degree || edu.field) && (
            <div style={{
              ...
              color: 'rgba(0,229,255,0.7)', marginBottom: 6,
            }}>
        ...
        {isEditMode && (
          <button onClick={() => onEdit(edu)} style={{
            flexShrink: 0,
            background: 'rgba(0,229,255,0.06)', border: '1px solid rgba(0,229,255,0.2)',
            color: 'var(--cyan)', padding: '4px 10px', borderRadius: '4px',
...
export default function Education() {
  ...
  return (
    <section id="education" style={{ background: 'rgba(255,255,255,0.01)' }}>
      <div style={{ padding: '100px 40px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {education.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
            {education.map(edu => (
              <EduCard key={edu.id} edu={edu} isEditMode={isEditMode} onEdit={setEditing} />
            ))}
          </div>
        )}
      </div>
      ...
      {editing && (
        <EditModal
          title={`edit — ${editing.institution}`}
          fields={EDU_FIELDS}
